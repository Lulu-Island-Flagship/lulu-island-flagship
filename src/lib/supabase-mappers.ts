...
export function mapOrderFromSupabase(data: Record<string, unknown>): Order {
  return {
    ...
    paymentOption: data.payment_option as Order["paymentOption"],
    ...
    // Auditoría 2026-07-30: columnas de migración 241 (Alipay/WeChat Pay) y
    // 152/245 (pago fraccionado 50/50) — antes no mapeadas, dejaban el
    // objeto Order incompleto.
    walletPaymentIntentId: (data.wallet_payment_intent_id as string | null) ?? null,
    walletAmountCollected: (data.wallet_amount_collected_cents as number) ?? 0,
    walletRefundedAmount: (data.wallet_refunded_amount_cents as number) ?? 0,
    installmentPlanSelected: (data.installment_plan_selected as boolean) ?? false,
    installmentFirstAmount: (data.installment_first_amount_cents as number | null) ?? null,
    installmentSecondAmount: (data.installment_second_amount_cents as number | null) ?? null,
    installmentSecondDueAt: data.installment_second_due_at as string | null | undefined,
    installmentSecondCapturedAt: data.installment_second_captured_at as string | null | undefined,
    installmentSecondPaymentIntentId: data.installment_second_payment_intent_id as string | null | undefined,
    installmentSecondAttempts: (data.installment_second_attempts as number) ?? 0,
    installmentSecondLastError: data.installment_second_last_error as string | null | undefined,
