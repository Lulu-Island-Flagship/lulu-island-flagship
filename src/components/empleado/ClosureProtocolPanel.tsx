...
function getExternalOptions(t: ReturnType<typeof useTranslations>): { value: ExternalConfirmationType; label: string }[] {
  return [
    { value: "client_verbal", label: t("externalOptions.verbal") },
    { value: "leader_audit", label: t("externalOptions.visual") },
    { value: "auditor_present", label: t("externalOptions.auditor") },
  ];
}

function getAltPaymentOptions(t: ReturnType<typeof useTranslations>): { value: AltPaymentMethod; label: string }[] {
  return [
    { value: "e_transfer", label: t("paymentOptions.etransfer") },
    { value: "cheque", label: t("paymentOptions.cheque") },
    { value: "cash", label: t("paymentOptions.cash") },
  ];
}
...
export function ClosureProtocolPanel({ orderId, noSmartphoneFlow }: ClosureProtocolPanelProps) {
  ...
  const t = useTranslations("closureProtocol");
  ...
  const save = async (patch: Record<string, unknown>) => {
    ...
    try {
      ...
      setSaveError(err.error || t("error.saveFailed"));
      return false;
    } catch (e) {
      console.error("Save closure error:", e);
      setSaveError(t("error.connection"));
      return false;
    } finally {
      setSaving(null);
    }
  ...
  const handleSignaturePhoto = async (file: File) => {
    ...
    try {
      ...
      if (uploadError) {
        ...
        setSaveError(t("error.uploadFailed"));
        return;
      }
      ...
      setSaveError(t("error.uploadConnection"));
    } finally {
  ...
  return (
    <div className="bg-white rounded-lg border p-4 space-y-4">
      <div className="flex items-center gap-2">
        ...
        <span className="font-semibold text-sm text-brand-ink">{t("title")}</span>
      </div>
      ...
      >
        ...
        {t("equipmentConfirmed")}
      </button>

      <div className="space-y-2">
        <p className="text-xs text-gray-500 font-medium">{t("externalConfirmationLabel")}</p>
        {getExternalOptions(t).map((opt) => (
          <button
            key={opt.value}
            type="button"
      ...
      {noSmartphoneFlow && (
        <div className="space-y-2 border-t pt-4">
          ...
          <div className="flex gap-2">
            {getAltPaymentOptions(t).map((opt) => (
              <button
                key={opt.value}
                type="button"
              ...
              <div>
                {altPaymentSignatureUrl ? (
                  ...
                  <label className="inline-flex items-center gap-1 text-xs text-gray-500 cursor-pointer hover:text-brand-gold-dark">
                    ...
                    <span>{uploadingSignature ? t("uploading") : t("photoRequired")}</span>
                    <input
                      type="file"
