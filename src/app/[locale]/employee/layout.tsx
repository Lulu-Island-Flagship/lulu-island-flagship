...
export default async function EmpleadoLayout({
  ...
  const rawPathname =
    headersList.get("x-pathname") || headersList.get("x-invoke-path") || `/${params.locale}/employee`;
  const pathname = isAllowedInternalPath(rawPathname) ? rawPathname : `/${params.locale}/employee`;
  ...
  // siempre next=/${safeLocale}/employee (el área genérica), descartando la
  ...
  if (!result.authorized || result.area !== "employee") {
