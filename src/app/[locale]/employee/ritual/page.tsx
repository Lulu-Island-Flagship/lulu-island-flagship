...
export default function ShiftRitualPage() {
  ...
  // corresponde. Ambos endpoints (/api/employee/readiness,
  // /api/employee/chemical-alert) ya existían testeados con sus funciones
  ...
  async function submitReadiness(e: React.FormEvent) {
    ...
    setReadinessResult("");
    try {
      const res = await fetch("/api/employee/readiness", {
        method: "POST",
  ...
  async function submitWellbeingCheckin() {
    ...
    try {
      const res = await fetch("/api/employee/chemical-alert", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        credentials: "include",
    ...
    async function load() {
      ...
      try {
        const [inicioRes, cierreRes] = await Promise.all([
          fetch("/api/employee/ritual/start", { credentials: "include" }),
          fetch("/api/employee/ritual/close", { credentials: "include" }),
        ]);
