-- Módulo de Cliente -- `client_properties` guarda cada propiedad
...
CREATE INDEX IF NOT EXISTS idx_client_properties_status ON client_properties (status);
...
-- de app directo.
DROP POLICY IF EXISTS "client_properties no direct access" ON client_properties;
CREATE POLICY "client_properties no direct access" ON client_properties
  FOR ALL USING (false) WITH CHECK (false);

COMMENT ON TABLE client_properties IS
